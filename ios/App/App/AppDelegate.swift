...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    // Función que se llama cuando el dispositivo se registra con éxito para notificaciones push
    func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        // Convierte el token de dispositivo en una cadena y envíalo a tu servidor
        let tokenString = deviceToken.map { String(format: "%02.2hhx", $0) }.joined()
        print("Device Token: \(tokenString)")
        self.sendDeviceTokenToServer(token: tokenString)
    }

    // Función que se llama si el registro para notificaciones push falla
    func application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
        print("Error al registrar para notificaciones push: \(error.localizedDescription)")
    }

    // Método personalizado para enviar el token al servidor
