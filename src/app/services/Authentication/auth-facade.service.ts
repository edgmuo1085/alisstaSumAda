...
export class AuthFacadeService {
          ...
          async (response: any) => {
            try {
              if (response?.error) {
                ...
                return resolve(false); // ⬅ LOGIN FALLÓ
              }
              ...
              await this.saveEncryptedUserInfo(userAuth);
              await this.saveUserAuthInfo(userAuth);
              ...
              resolve(true); // ⬅ LOGIN OK
            } catch (err) {
              console.error('Error handling login response:', JSON.stringify(err));
              await loading.dismiss();
              resolve(false); // ⬅ LOGIN FALLÓ
            }
          },
          async err => {
            this.configSv.isLogged = false;
  ...
  private async saveEncryptedUserInfo(userAuth: any): Promise<void> {
    try {
      ...
      if (!existingEncryptedInfo) {
        ...
        this.encryptInfoUser = CryptoJS.AES.encrypt(JSON.stringify(userAuth), this.secretKey.trim()).toString();
   ...
   */
  private async maybeAskEnableBiometric(employerID: number | string, userID: string, password: string): Promise<boolean> {
  ...
  async loginWithBiometric(): Promise<void> {
    ...
    console.log("LOS CREDS..!!!: ", creds)
    console.log("LOS CREDS JSON..!!!: ", JSON.stringify(creds, null, 2))
    if (!creds) {
      const alert = await this.alertCtrl.create({
    ...
    const employerID = parseInt(creds.nit);
