...
export class CompaniesService {
   ...
   */
   ...
   save(): Observable<any> {
    ...
    return this.http.post(url, this.company).pipe(
      tap(async (r: any) => {
        if (!this.isSuccessfulResponse(r)) {
          return;
        }
  ...
  private isSuccessfulResponse(response: any): boolean {
    return response.split(';')[0] === 'true';
  }

  private async removeCompanyFromLists(companyId: number): Promise<void> {
    const indexC = (await this.companies).findIndex(c => c.Pk_Id_AS_004_Empresas_AMigrar === companyId);
    if (indexC !== -1) {
      this._companies.splice(indexC, 1);
      await this.setCompanies(this._companies);
    }

    const indexM = (await this.updatedCompanies).findIndex(m => m.Pk_Id_AS_004_Empresas_AMigrar === companyId);
    if (indexM !== -1) {
