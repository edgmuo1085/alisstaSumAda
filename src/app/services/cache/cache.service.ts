...
export class CacheService {
  ...
  private transformActivities(activities: any[]): any[] {
    return activities.map(activity => ({
      id: activity.id,
      codigo: activity.idActividad,
      Cantidad: activity.horasAEjecutar,
      UnidadMedida: activity.unidadMedida,
      Cobertura: activity.coverage,
      EjecucionReal: parseInt(activity.horasEjecutadas, 10),
      NumeroAutorizacion: '',
      NumeroActividad: '',
      Descripcion: activity.descripcionActividad,
      ObservacionesSeguimiento: activity.siniestro?.Observaciones ?? null,
      Recomendaciones: this.transformRecomendaciones(activity.siniestro?.Recomendaciones),
      AdjuntarDocumentosTecnicos: activity.AdjuntarDocumentosTecnicos,
  ...
  private setTypeAdviceInfo() {
    if (this.typeAdvice.type === 'Especifica') {
      this.typeAdviceSelectedEspec = {
        ...
        AE_Fecha: this.formatDate(this.typeAdvice.fechaInicialAsesoria),
        AE_HoraInicio: this.formatTime(this.typeAdvice.startHour),
        AE_HoraFin: this.formatTime(this.typeAdvice.endHour),
        AE_HorasTotales: this.typeAdvice.totalHours,
      };
      this.typeAdviceSelectedProject = { AP: false, AP_Mes: '', AP_Anio: '' };
    } else {
      this.typeAdviceSelectedEspec = { AE: false, AE_Fecha: '', AE_HoraInicio: '', AE_HoraFin: '', AE_HorasTotales: '' };
