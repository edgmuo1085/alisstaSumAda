...
export class RecommendationPage {
  ...
  private initForm(): void {
    const numeroSiniestroControl = new UntypedFormControl({
      value: this.actividad.siniestro.Id_Siniestro,
      disabled: true,
    });

    const numeroIdentificacionControl = new UntypedFormControl({
      value: this.actividad.siniestro.Doc_Empleado,
      disabled: true,
    });

    const nombreTrabajadorControl = new UntypedFormControl({
    ...
    const fechaOcurrenciaControl = new UntypedFormControl({
    ...
    const fechaRadicacionControl = new UntypedFormControl({
    ...
    const fechaRemisionControl = new UntypedFormControl({
    ...
    const fechaProyectadaControl = new UntypedFormControl({
      value: isInvalid ? 'Sin información' : fechaProyectada.format('DD/MM/YYYY'),
      disabled: true,
    });

    const observacionesControl = new UntypedFormControl(this.actividad.siniestro.Observaciones, [Validators.required]);
