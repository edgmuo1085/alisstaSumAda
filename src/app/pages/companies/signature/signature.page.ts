...
export class SignaturePage implements OnInit {
  ...
  drawStart() {
    ...
    console.log('begin drawing Componente aun desconocido');
  }

  /**
   * Maneja el evento de culminación de dibujado en el componente de firma.
   */
  drawComplete(signaturePadComponent: string, control: string): void {
    const firma = signaturePadComponent.split(',');
   ...
   */
  clear(signaturePadComponent: SignaturePadComponent, control: string) {
  ...
  private initForm(): void {
    const responsableControl = new UntypedFormControl(undefined, [Validators.required]);
    const tipoDocumentoControl = new UntypedFormControl({ value: undefined, disabled: true });
    const documentoControl = new UntypedFormControl({ value: undefined, disabled: true });
    const correoControl = new UntypedFormControl({ value: undefined, disabled: true });
    const cargoControl = new UntypedFormControl({ value: undefined, disabled: true });
    const qrControl = new UntypedFormControl(false);
    const firmaARLControl = new UntypedFormControl(undefined, [Validators.required]);
    const codigoVerificacionControl = new UntypedFormControl(undefined, [Validators.required]);
    const firmaEmpresaControl = new UntypedFormControl(undefined, [Validators.required]);
