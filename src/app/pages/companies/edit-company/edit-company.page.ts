...
export class EditCompanyPage {
   ...
   */
  private readonly NUMBER_ONLY = /^\d{10}$/;
  ...
  private initForm(): void {
    ...
    const razonSocialEmpresaControl = new UntypedFormControl({ value: this.company.strRazonSocial, disabled: true });

    const idSedeControl = new UntypedFormControl({ value: this.company.eDSedesActualizadas.strNombreSede, disabled: true });

    const direccionSedeControl = new UntypedFormControl(this.company.eDSedesActualizadas.strDireccion, [Validators.required]);

    const indicadorZonaControl = new UntypedFormControl(this.company.eDSedesActualizadas.srtIndicadorZona, [Validators.required]);
    ...
    const departamentoSedeControl = new UntypedFormControl(departamento, [Validators.required]);
    ...
    const municipioSedeControl = new UntypedFormControl(municipio, [Validators.required]);
    ...
    const celularControl = new UntypedFormControl(this.company.strCelular, [
      Validators.pattern(this.CELLPHONE_REGEX),
      Validators.pattern(this.NUMBER_ONLY),
      Validators.maxLength(10),
    ]);

    const telefonoNotificacionControl = new UntypedFormControl(this.company.strTelefono, [
      Validators.pattern(this.TEL_REGEX),
      Validators.pattern(this.NUMBER_ONLY),
      Validators.maxLength(10),
    ]);

    const correoControl = new UntypedFormControl(this.company.strCorreoElectronico, [Validators.required, Validators.pattern(this.EMAIL_REGEX)]);

    const codigoPostalControl = new UntypedFormControl(this.company.eDSedesActualizadas.strCodigoPostal, [Validators.pattern(this.ZIPCODE_REGEX)]);
