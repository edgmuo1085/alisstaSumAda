...
export class EditContactPage {
  ...
  private initForm(): void {
    ...
    const representacionControl = new UntypedFormControl(representacion, [Validators.required]);

    const tipoDocumento = this.TIPOS_DOCUMENTO.find(d => d.value === contact.intTipoDocumento);
    const tipoDocumentoControl = new UntypedFormControl(tipoDocumento, [Validators.required]);

    const numeroDocumentoControl = new UntypedFormControl(contact.strNumeroDocumento, [Validators.required]);
    const primerNombreControl = new UntypedFormControl(contact.strPrimerNombre, [Validators.required]);
    const segundoNombreControl = new UntypedFormControl(contact.strSegundoNombre);
    const primerApellidoControl = new UntypedFormControl(contact.strPrimerApellido, [Validators.required]);
    const segundoApellidoControl = new UntypedFormControl(contact.strSegundoApellido);

    const sexo = this.TIPOS_SEXO.find(s => s.value === contact.strSexo);
    const sexoControl = new UntypedFormControl(sexo, [Validators.required]);

    const correoControl = new UntypedFormControl(contact.strEmail, [Validators.required, Validators.pattern(this.EMAIL_REGEX)]);
    const celularControl = new UntypedFormControl(contact.strCelular, [Validators.required, Validators.pattern(/^3\d{9}$/)]);
    const telefonoControl = new UntypedFormControl(contact.strTelefono, [Validators.required, Validators.pattern(/^\d{7,10}$/)]);
