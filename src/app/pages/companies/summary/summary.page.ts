...
export class SummaryPage {
  ...
  private initForm(): void {
    ...
    const identificacionEmpresaControl = new UntypedFormControl({ value: identificacionEmpresa, disabled: true });

    const razonSocialControl = new UntypedFormControl({ value: this.company.strRazonSocial, disabled: true });

    const idSedeControl = new UntypedFormControl({ value: this.company.eDSedesActualizadas.strNombreSede, disabled: true });

    const direccionControl = new UntypedFormControl({ value: this.company.strDireccion, disabled: true });

    const indicadorZonaControl = new UntypedFormControl({ value: this.company.eDSedesActualizadas.srtIndicadorZona, disabled: true });
    ...
    const departamentoSedeControl = new UntypedFormControl({ value: departamento, disabled: true });
    ...
    const municipioSedeControl = new UntypedFormControl({ value: municipio, disabled: true });

    const estadoSedeControl = new UntypedFormControl({ value: this.company.eDSedesActualizadas.strNombreSede, disabled: true });

    const actividadEconomica = `${this.company.strCodigoActividadEconomica} ${this.company.strActividadEconomicaNombre}`;
    const actividadEconomicaControl = new UntypedFormControl({ value: actividadEconomica, disabled: true });

    const sectorEconomicoControl = new UntypedFormControl({ value: this.company.strDescripcionSectorEconomico, disabled: true });

    const celularControl = new UntypedFormControl({ value: this.company.strCelular, disabled: true });

    const telefonoNotificacionControl = new UntypedFormControl({ value: this.company.strTelefono, disabled: true });

    const correoControl = new UntypedFormControl({ value: this.company.strCorreoElectronico, disabled: true });

    const codigoPostalControl = new UntypedFormControl({ value: this.company.eDSedesActualizadas.strCodigoPostal, disabled: true });

    const identificacionRepresentante = `${this.company.strSiglaRepresentanteLegal} ${this.company.strNumeroDcto_Representant_Legal}`;
    const identificacionRepresentanteControl = new UntypedFormControl({ value: identificacionRepresentante, disabled: true });

    const representanteLegalControl = new UntypedFormControl({ value: this.company.strNombre_Representant_Legal, disabled: true });
    ...
    contacts.forEach((c: any, i) => {
      ...
      const representacionControl = new UntypedFormControl({ value: representacion, disabled: true });

      const tipoDocumento = this.TIPOS_DOCUMENTO.find(d => d.value === c.intTipoDocumento);
      const tipoDocumentoControl = new UntypedFormControl({ value: tipoDocumento, disabled: true });

      const numeroDocumentoControl = new UntypedFormControl({ value: c.strNumeroDocumento, disabled: true });

      const primerNombreControl = new UntypedFormControl({ value: c.strPrimerNombre, disabled: true });

      const segundoNombreControl = new UntypedFormControl({ value: c.strSegundoNombre, disabled: true });

      const primerApellidoControl = new UntypedFormControl({ value: c.strPrimerApellido, disabled: true });

      const segundoApellidoControl = new UntypedFormControl({ value: c.strSegundoApellido, disabled: true });

      const sexo = this.TIPOS_SEXO.find(s => s.value === c.strSexo);
      const sexoControl = new UntypedFormControl({ value: sexo, disabled: true });

      const correoControl = new UntypedFormControl({ value: c.strEmail, disabled: true });

      const celularControl = new UntypedFormControl({ value: c.strCelular, disabled: true });

      const telefonoControl = new UntypedFormControl({ value: c.strTelefono, disabled: true });
